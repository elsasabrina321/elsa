            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header bg-indigo" >
                        <h2>
                             <i class="material-icons">add</i> Tambah Rule <small>Gunakan halaman ini untuk menambahkan data Rule baru.</small>
                        </h2>
                    </div>
                    <div class="body">
                        <form action="{{route('rule.store')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                @if ($errors->has('jurusan_id'))
                                     <small style="color: red">{{ $errors->first('jurusan_id') }}</small><br>
                                @endif
                            </div>
                            <label for="arr">Karakteristik</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <select id='keep-order' name="arr[]" multiple='multiple'>
                                        @foreach($karakteristik as $karak)
                                            <option value="{{$karak->id}}">{{$karak->kode}} - {{$karak->karakteristik}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @if ($errors->has('arr'))
                                     <small style="color: red">{{ $errors->first('arr') }}</small><br>
                                @endif
                            </div>
                            ...
                            <br>
                            <a href="{{route('rule.index')}}" class="btn btn-danger m-t-15 waves-effect"><i class="material-icons">cancel</i> batal</a>
