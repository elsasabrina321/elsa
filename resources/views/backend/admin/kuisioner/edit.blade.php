            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header bg-indigo" >
                        <h2>
                             <i class="material-icons">edit</i> Edit Kuisioner Kondisi Keluarga <small>Gunakan halaman ini untuk mengedit data kuisioner baru.</small>
                        </h2>
                    </div>
                    <div class="body">
                        <form action="{{route('soal.update',$cari->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <label for="karakteristik">Soal</label>
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" id="soal" value="{{$cari->soal}}" name="soal" class="form-control" placeholder="Soal">
                                </div>
                                @if ($errors->has('soal'))
                            ...
                            <div class="form-group">
                                <div class="form-line {{ $errors->has('keterangan') ? 'focused error' : '' }}">
                                    <input type="text" value="{{$cari->keterangan}}" id="keterangan" name="keterangan" class="form-control" placeholder="Keterangan">
                                </div>
                            ...
                            <button type="submit" class="btn btn-primary m-t-15 waves-effect"><i class="material-icons">edit</i> update</button>
                        </form>
                    </div>
                </div>
            </div>
