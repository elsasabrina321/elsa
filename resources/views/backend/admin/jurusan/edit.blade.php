            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header bg-indigo" >
                        <h2>
                             <i class="material-icons">edit</i> Edit Jurusan : {{$cari->jurusan}} <small>Gunakan halaman ini untuk mengedit data jurusan baru.</small>
                        </h2>
                    </div>
                    <div class="body">
                        <form action="{{route('jurusan.update',$cari->id)}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <label for="kode">Jurusan</label>
                            <div class="form-group">
                                <div class="form-line {{ $errors->has('jurusan') ? 'focused error' : '' }}">
                                    <input type="text" value="{{ $cari->jurusan }}" id="jurusan" name="jurusan" class="form-control" placeholder="Nama Jurusan">
                                </div>
                            ...
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" id="karakteristik" value="{{ $cari->deskripsi }}" name="deskripsi" class="form-control" placeholder="Deskripsi Jurusan">
                                </div>
                                @if ($errors->has('deskripsi'))
                                     <small style="color: red">{{ $errors->first('deskripsi') }}</small><br>
                                @endif
                            </div>
                            <img style="width: 200px" src="{{asset('storage/gambar/'.$cari->gambar)}}" class="img-thumbnail">
                                <br><br>
